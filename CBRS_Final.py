...
# Slice first 1000 rows and 1000 columns or full dataset
...
eval_df_mini = labels.head(9999).reset_index(drop=True).drop(['date'], axis=1)  # Use for sample
eval_df = labels.reset_index(drop=True).drop(['date'], axis=1)  # Use for entire dataset
...
# Iterate over each item and check whether the recommendations match the purchase
df_out['check'] = 0
...
# Group sessions where a prediction was the purchased item and calculate percentage over total
